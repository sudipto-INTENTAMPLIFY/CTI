...
const scripts=['CONVERSION_LAYER_CONFIG.js','RC11_LOADER.js','RC11_LIVE_CONTENT.js','CTI_DYNAMIC_CMS.js'];
...
for(const f of ['CONVERSION_LAYER_CONFIG.js','RC11_LOADER.js','RC11_RUNTIME.js','RC11_LIVE_CONTENT.js','CTI_DYNAMIC_CMS.js','CTI_ADMIN.js','.htaccess','admin.html'])await copyFile(f,`dist/${f}`);
await copyFile('data/evidence-graph.json','dist/data/evidence-graph.json');
for(const file of ['_lib.php','website-leads.php','signals.php','content.php','vendors.php','pricing.php','.htaccess'])await copyFile(`api/${file}`,`dist/api/${file}`);
...
for(const required of ['dist/pricing.html','dist/CONVERSION_LAYER_CONFIG.js','dist/admin.html','dist/CTI_ADMIN.js','dist/api/content.php','dist/api/vendors.php','dist/api/pricing.php','dist/api/website-leads.php','dist/.htaccess','dist/api/.htaccess'])await access(required);
console.log(`RC9 final staging artifact built: ${pages.length} pages + governed pricing + CMS admin/API + runtime/evidence assets.`);